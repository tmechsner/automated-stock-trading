__author__ = 'vlosing'
from SAMkNN.SAMKNN.BaseClassifier import BaseClassifier
import numpy as np
...
class SAMKNN(BaseClassifier):
    ...
    def predictByAllMemories(self, sample, label, distancesSTM):
        ...
        if len(self._STMLabels) > 0:
            ...
            correctSTM = np.sum(self.STMPredHistory) if predictedLabelSTM in self._classes else 0
            correctLTM = np.sum(self.LTMPredHistory) if predictedLabelLTM in self._classes else 0
            correctCM = np.sum(self.CMPredHistory) if predictedLabelCM in self._classes else 0
    ...
    def partial_fit(self, samples, labels, classes):
        """Processes a new sample."""
        self._classes = np.union1d(self._classes, classes)
    ...
    def alternateFitPredict(self, samples, labels, classes, verbose=False):
        ...
        for i in range(len(labels)):
            if verbose and (i+1)%(len(labels)/20) == 0:
